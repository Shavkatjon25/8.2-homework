...
function SongiQism() {
  ...
  const uzru=(e)=>{
      if (tl=='uz') {
        dispach(Omborchi.actions.fuz(e))
      }
      if (tl=='ru') {
        dispach(Omborchi.actions.fru(e))
      }
      if (tl=='eng') {
        dispach(Omborchi.actions.feng(e))
      }
  }
    ...
    <div className="form ikki">
      ...
      <div className="div">
        ...
        <form action="" className="from_div1">
          ...
          <input type="url" value={data} onChange={(e)=>dispach(Omborchi.actions.furl(e.target.value))} className="input" placeholder="Link" />
            <div>
              <div className="habar_div">
                <h3 className="habar">Habar</h3>
                <button className="sh"><span style={{border:'1px solid blue', color:'blue', width:'16px', height:'16px', boxSizing:'border-box', borderRadius:'5px', textAlign:'center'}}>+</span> <span>Shablon holatida saqlash</span></button>
              </div>
              <div className="tillar">
                <button className={`${tl=='uz' ? 'tillar_btn':'bn'}`} onClick={(e)=>{useTl('uz'); e.preventDefault()} }>O'zbekcha</button>
